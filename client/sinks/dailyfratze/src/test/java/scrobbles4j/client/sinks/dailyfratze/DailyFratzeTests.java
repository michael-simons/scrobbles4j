...
class DailyFratzeTests {
	...
	void shouldRecreateOldFormat(WireMockRuntimeInfo wmRuntimeInfo) {
		...
		sink.init(
				Map.of("bearerToken", "Braunbaer", "endpoint", wmRuntimeInfo.getHttpBaseUrl() + "/api/scrobbles/new"));

		var track = new Track(new Artist("Swiss + Die Andern"), new Genre("Punk"), "Orphan",
				"Punkrock Karate [mit Mortis & Mal Élevé]", 2021, 291, 40, 1, "ripped & encoded by MJS",
				new TrackNumber(9, 11), new DiscNumber(1, 2), false);
		...
		verify(1,
				postRequestedFor(urlPathEqualTo("/api/scrobbles/new"))
