...
on handle_url(newUrl)
	...
	repeat with i from 1 to the count of arguments
		...
		if (item 1 of argument is equal to "album") then
			set albumToPlay to decodeText(item 2 of argument)
		end if
	...
	try
		...
		if (albumToPlay is not "") then
			write ({artistToPlay, albumToPlay} as text) to fhandle as Çclass utf8È
			close access fhandle
			do shell script ("more " & posixtmpfile & " | shortcuts run \"Push album\"")
		else
